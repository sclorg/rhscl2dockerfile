...
```
docker run -d --name mysql_database -p 3306:3306 {{ container }}
```
...
```
docker run -d --name mysql_database -v /host/data:/var/lib/mysql/data {{ container }}
```
...
```
docker run -d --name mysql_database -e MYSQL_USER=user -e MYSQL_PASSWORD=pass -e MYSQL_DATABASE=db -p 3306:3306 {{ container }}
```
